...
# Función para la secuencia vibracional (hormiguita)
def secuencia_vibracional():
...
def cambiar_fondo(nueva_imagen):
    ...
    global bg_image  # Usamos una variable global para la imagen de fondo
    image_path = fr"C:\Users\felip\OneDrive\Escritorio\9no_semestre\pds\cunero\images_ssindex\{nueva_imagen}"
...
def auditivos(): 
    ...
    cambiar_fondo("3.png")  # Cambia a la imagen 3 cuando se presiona "Auditivos"
    ocultar_botones() 
    ...
    b_music = tk.Button(root, text="musica relajante", command=volver_menu_principal, width=15, height=2)
    b_music.place(x=x_center-210, y=y_start+290)
    b_nat = tk.Button(root, text="naturaleza", command=volver_menu_principal, width=15, height=2)
    b_nat.place(x=x_center+280, y=y_start+290)
    b_heart = tk.Button(root, text="relajacion", command=volver_menu_principal, width=15, height=2)
...
canvas.create_image(0, 0, image=bg_image, anchor=tk.NW)
canvas.image = bg_image  # Guarda la referencia inicial
